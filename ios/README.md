# SDR++ iOS (client-only) — fork

This branch is a hard fork of SDR++ for iOS / iPadOS. It is **not** intended
to merge back to upstream — desktop platform code (GLFW, Windows, Linux, USB
hardware sources, Discord plugin, packaging scripts) has been removed
entirely. To pull in upstream bugfixes, cherry-pick or rebase the relevant
...
1. **Vendored ImGui Metal backend.** Drop `imgui_impl_metal.{h,mm}` from the
   Dear ImGui distribution into `core/backends/ios/imgui/`. See the README
   there for why we don't pin a copy.
2. **Cross-compiled deps.** Build `fftw3f`, `libzstd`, and (optionally) `volk`
...
3. **Touch UX.** The current backend forwards a single touch as the left
   mouse button. A real iOS port wants pinch-to-zoom on the waterfall and
   long-press → right-click for context menus.
4. **Soft keyboard.** `iosWantsKeyboard()` is plumbed but the host shell
   doesn't yet present a UITextField to capture text input.
5. **App lifecycle.** `applicationWillResignActive` should call
...
## Adding a module from another worktree

When a module is finished in a separate Claude session / branch:

1. From this branch: `git cherry-pick <commit-on-other-branch>`
