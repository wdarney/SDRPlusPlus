...
Development branch: `codex/channel-bank-multi-receiver-scan`. Combined build baseline: `integration/main`.

Initial feature commit: `2932708f995e076ea1eaab3f759d052117f7decf`. Receiver and VFO activity display: `6aa89dbb`. Handoff and allocator fixture cleanup: `5737ee20`. Build the integration baseline to include these updates and configurable scan settling.
...
From a clean checkout of the integration baseline, use a fresh build directory. A focused macOS configuration is:
...
cmake --build build-multi-receiver-scan --target \
  ...
  channel_bank_multi_receiver_allocator_test channel_bank_scan_readiness_test -j8
...
Integration validation on 2026-09-23 combined baseline `0181f692` with feature tip `5737ee20` in a fresh ARM64 Release build. The `sdrpp` and `channel_bank` targets built successfully; allocator and scan-readiness CTests passed (2/2), with allocator assertions enabled in Release. The merge required no source-conflict resolution and preserves the configurable settling controls. This integration check did not package, install, or run an SDR hardware test; application packaging and RF checks remain with the separate build session.
