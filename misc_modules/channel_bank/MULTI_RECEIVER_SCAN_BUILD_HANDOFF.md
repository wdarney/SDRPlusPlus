...
Feature commit: `2932708f995e076ea1eaab3f759d052117f7decf`

Base: `origin/integration/main` at `7ad6434ce6019b2933ba997a906e25528c8d8c41`

This feature adds **Multi-Receiver Scan** to desktop Channel Bank. One user-selected SDR++ source scans the configured ranges for activity. A separate dispatch thread assigns each detected frequency to a selected transmission source, reusing an active receiver when its current IQ window covers the frequency and otherwise starting the first eligible idle receiver. Each assignment uses the existing Channel Bank channel, demodulation, recording, playback, transcription, and blocking paths. Discovery proceeds without waiting for receiver setup.
...
The following source modules supply the optional callbacks and can be selected for the transmission pool: `rtl_sdr_source`, `airspy_source`, `airspyhf_source`, `soapy_source`, and `rx888_source`. The desktop Channel Bank changes are in `misc_modules/channel_bank/src/main.cpp`; allocator logic and its unit test are under that module. The iOS Channel Bank implementation was not changed.
...
The user will perform application and SDR hardware testing. Useful checks are immediate discovery advancement after detection, multiple VFOs on one receiver's IQ span, idle-receiver allocation, no-capacity behavior, shared blocking, maximum-monitor suppression and carrier-clear reset, receiver disconnect, and clean stop/restart. Do not deploy or alter an installed SDR++ app solely from this handoff.
