...
// On iOS, use the AVAudioPlayer bridge instead of writing to the monitor
// stream (which requires a second CoreAudio sink that can't be created —
// kAudioUnitSubType_RemoteIO is a device-wide singleton).
...
public:
...
#if defined(TARGET_OS_IPHONE) && TARGET_OS_IPHONE
        // On iOS the "..." file picker is a no-op and container UUIDs change on
        // reinstall, so any absolute path saved in config may be stale.
        // Always resolve to the current recordings dir so the path is always valid.
        folderSelect.setPath(root + "/recordings");
#else
        // On desktop fall back to %ROOT%/recordings if the saved path is gone.
    ...
    void start() {
        ...
        // Auto-route the monitor stream to the "Audio" sink on first use so
        // iOS playback works without the user having to open the sink config.
        {
            core::configManager.acquire();
            bool hasConfig = core::configManager.conf["streams"].contains(name + "_monitor");
            core::configManager.release();
            if (!hasConfig) {
                sigpath::sinkManager.setStreamSink(name + "_monitor", "Audio");
                core::configManager.acquire();
                core::configManager.conf["streams"][name + "_monitor"]["sink"]   = "Audio";
                core::configManager.conf["streams"][name + "_monitor"]["volume"] = 1.0f;
                core::configManager.conf["streams"][name + "_monitor"]["muted"]  = false;
...
private:
    ...
    void analyzeSpectrum() {
        ...
        if (retuneFlag.load()) {
            ...
            slotVotes.clear();
            manualVotes.clear();
            slotMeanHistory.clear();
            confirmedSpurs.clear();
        ...
        // How many slots at each end are excluded from floor estimation AND
        // from detection.  bwUsage controls the user-facing margin, but we
        // also enforce a hard minimum: at least 150 kHz (≈ 6 slots at 25 kHz
        // spacing) regardless of bwUsage.  SDR IF-filter transition bands are
        // typically 5–15% of the sample rate wide; 150 kHz covers even a
        // modest 1–2 MHz SDR bandwidth.  Without this, bwUsage values above
        // ~85% leave so few edge slots excluded that filter rolloff spills in.
        const int edgeSkipBw  = (int)std::round(numSlots * (1.0f - bwUsage) / 2.0f);
        const int edgeSkipMin = std::max(2, (int)std::ceil(150000.0 / channelSpacing));
        const int edgeSkip    = std::max(edgeSkipBw, edgeSkipMin);

        // Global noise floor — 20th percentile of CENTER slot means (bwUsage
        // fraction).  Edge slots (edgeSkip) are excluded so filter rolloff
        // doesn't inflate the floor estimate.
        {
        ...
        // The global floor above handles temporal stability (median-over-frames);
        // this adapts spatially to hardware noise tilt, IQ imbalance, and filter
        // rolloff that make one side of the band systematically noisier than the
        // other. Without it, signals on the noisier half are missed even though
        // they are clearly visible in the waterfall.
        const int NOISE_WIN = std::max(3, numSlots / 5);
        std::vector<float> localFloor(numSlots);
        for (int s = 0; s < numSlots; s++) {
            int lo2 = std::max(0, s - NOISE_WIN);
            int hi2 = std::min(numSlots - 1, s + NOISE_WIN);
            // Don't let edge-zone windows borrow power values from the centre
            // band.  Centre slots have much lower power (no filter rolloff) so
            // mixing them into an edge slot's window drags the 20th-percentile
            // floor down toward centre-band noise while the edge slot's mean
            // stays elevated — making mean/floor look like a strong signal and
            // triggering false detections regardless of SNR threshold.
            if (s < edgeSkip)             hi2 = std::min(hi2, edgeSkip - 1);
            if (s >= numSlots - edgeSkip) lo2 = std::max(lo2, numSlots - edgeSkip);
            std::vector<float> win(slotMeans.begin() + lo2, slotMeans.begin() + hi2 + 1);
            std::sort(win.begin(), win.end());
            int idx = std::max(0, (int)(win.size() * 0.20f) - 1);
            localFloor[s] = std::max(win[idx], 1e-30f);
        }

        // ── Spur / birdie suppression ────────────────────────────────────────
        // Hardware artifacts (DC offset, IQ-imbalance mirror images, ADC birdies)
        // produce constant-power signals that are always above threshold with
        // near-zero power variance.  Real PTT transmissions (AM, NFM, SSB)
        // always have silent gaps between overs, causing slotMeans to drop to
        // the noise floor at least once every few seconds.
        //
        // Strategy: keep a ~2 s rolling history of slotMeans per slot.  If the
        // slot has been above threshold for the whole window AND the ratio of
        // its max to min power is less than SPUR_RATIO (≈ 1.76 dB), flag it as
        // a confirmed spur.  Spur slots don't accumulate votes, don't spawn
        ...
        // detected.
        {
            ...
            confirmedSpurs.clear();
            for (int s = 0; s < numSlots; s++) {
                auto& hist = slotMeanHistory[s];
                hist.push_back(slotMeans[s]);
                if ((int)hist.size() > SPUR_HISTORY) hist.pop_front();

                if ((int)hist.size() < SPUR_HISTORY) continue; // still warming up
        ...
        if (manualMode) {
            ...
            // Per-frequency detection halfwidth.
            // Start from the same limit as AUTO mode (channelSpacing*0.4 or
            // DETECT_BW_HZ/2, whichever is smaller), then tighten further to
            // 45% of the distance to the nearest neighbour.  This prevents a
            // strong signal on one channel from bleeding into a closely-spaced
            // adjacent channel without widening the window for isolated channels
            // (which would capture more filter rolloff and reintroduce ghost
            // signals at the band edges).
            const double maxHalfHz = std::min(channelSpacing * 0.4, DETECT_BW_HZ / 2.0);
            std::vector<double> perFreqHalfHz(localFreqs.size(), maxHalfHz);
            {
                std::vector<std::pair<double,int>> inBand;
                for (int i = 0; i < (int)localFreqs.size(); i++)
                    if (std::abs(localFreqs[i] - lastKnownCenter) < lastKnownSr / 2.0)
                        inBand.push_back({localFreqs[i], i});
                std::sort(inBand.begin(), inBand.end());
                for (int k = 0; k < (int)inBand.size(); k++) {
                    double half = maxHalfHz;
                    if (k > 0)
                        half = std::min(half, (inBand[k].first - inBand[k-1].first) * 0.45);
                    if (k < (int)inBand.size() - 1)
                        half = std::min(half, (inBand[k+1].first - inBand[k].first) * 0.45);
                    perFreqHalfHz[inBand[k].second] = half;
                }
            }
            ...
            for (int i = 0; i < (int)localFreqs.size(); i++) {
                ...
                int halfBins2 = std::max(1, (int)std::round(perFreqHalfHz[i] / binHz));

                // Signal power: mean of bins inside the detection window.
                ...
                // Local reference floor: mean of bins immediately outside the
                // detection window (one half-width on each side).  Because these
                // bins are adjacent they track any spillover from a nearby strong
                // transmission.  Comparing the signal mean against this local
                // reference (rather than the global slot floor alone) means the
                // SNR check requires the target channel to be genuinely stronger
                // than its own immediate background — not just stronger than the
                // band-average noise floor — which suppresses bleed detections
                // without needing a dominance-ratio heuristic.
                {
                    float refSum = 0.0f; int refN = 0;
                    int rlo = std::max(0,          centerBin - 2 * halfBins2);
                    int rhi = std::min(FFT_SIZE-1, centerBin - halfBins2 - 1);
                    for (int b = rlo; b <= rhi; b++) { refSum += power[b]; refN++; }
                    rlo = std::max(0,          centerBin + halfBins2 + 1);
                    rhi = std::min(FFT_SIZE-1, centerBin + 2 * halfBins2);
                    for (int b = rlo; b <= rhi; b++) { refSum += power[b]; refN++; }
                    if (refN > 0) {
                        float refFloor = refSum / (float)refN;
                        // Use whichever floor is higher — the slot-based estimate
                        // handles global noise tilt; the reference floor handles
                        // local interference from adjacent channels.
                        int nearestSlot = std::clamp(
                            (int)std::round((freqOffset / channelSpacing) + (numSlots - 1) / 2.0),
                            0, numSlots - 1);
                        float effectiveFloor = std::max(refFloor, localFloor[nearestSlot]);
                        bool above = (mean > effectiveFloor * snrLinear);
                        int& v = manualVotes[i];
                ...
                int nearestSlot = std::clamp(
                    (int)std::round((freqOffset / channelSpacing) + (numSlots - 1) / 2.0),
                    0, numSlots - 1);
                // localFloor[nearestSlot] is now correctly estimated for edge
                // slots (window clamped to stay within the edge zone, so it
                // reflects the actual elevated rolloff power rather than the
                // artificially-low centre-band level).  A real signal adds SNR
                // on top of that elevated floor; pure rolloff alone does not.
                bool above = (mean > localFloor[nearestSlot] * snrLinear);
                int& v = manualVotes[i];
                v = above ? std::min(v + 1, MAX_VOTES) : std::max(v - 1, 0);
                if (v >= SPAWN_VOTES) newDetected.insert(i);
            }
        ...
        // Vote on each slot against its local noise floor (see localFloor above).
        // Edge slots (inside edgeSkip) and confirmed spurs are treated as
        // below-threshold: their votes drain to zero, tearing down any existing
        // channel and preventing re-spawning.  Edge slots are excluded because
        // filter rolloff at the band boundary produces elevated power that is
        // indistinguishable from a real signal until the spur history fills (~2 s);
        // this immediate exclusion covers the gap.
        for (int s = 0; s < numSlots; s++) {
            bool atEdge = (s < edgeSkip || s >= numSlots - edgeSkip);
            bool aboveThreshold = !atEdge
                                  && (slotMeans[s] > localFloor[s] * snrLinear)
                                  && !confirmedSpurs.count(s);
    ...
    void initSlot(ChannelSlot& slot, int gridIdx, int numSlots, double peakOffsetHz, double exactOffsetHz = NAN) {
        ...
        // For USB/LSB use the sideband width as the VFO bandwidth so the
        // RxVFO's low-pass filter matches the sideband (±1400 Hz), exactly
        // as the standard radio module does.  Using channelSpacing (25 kHz)
        // here lets in ±12.5 kHz of noise; after ComplexToReal that noise
        // folds back into the 0–2800 Hz audio band and sounds like the
        // signal is off-frequency even though the carrier is correct.
        // AM/FM use the full channel width so both sidebands are captured.
        const double bw = (demodMode == DEMOD_USB || demodMode == DEMOD_LSB)
                          ? ssbBw : channelSpacing;
        ...
        // BFO trim (ssbBfoHz): positive = raise pitch = lower VFO frequency.
        // Applied in both manual and auto mode so the user has one control
        // for SDR PPM / transmitter frequency error compensation.
        // Auto mode: centroid already tracks the actual signal position, so BFO
        // is a fine-tune.  Manual mode: entered frequency is nominal (no PPM
        // correction), so BFO is the primary way to zero-beat a USB/LSB signal.
        const double vfoOff = isManual
            ? (demodMode == DEMOD_USB ? offset + ssbBw / 2.0 - (double)ssbBfoHz
             : demodMode == DEMOD_LSB ? offset - ssbBw / 2.0 + (double)ssbBfoHz
             : offset)
...
#if defined(TARGET_OS_IPHONE) && TARGET_OS_IPHONE
        // iOS: kAudioUnitSubType_RemoteIO is a singleton — a second CoreAudioSink
        // instance for the monitor stream cannot be created, so the normal
        // monitorStream.swap() path produces no audio. Use AVAudioPlayer instead,
        // which reuses the existing active AudioSession without a new AudioUnit.
        backend::iosPlayRecordingFile(path.c_str(), &playbackRunning);
        return;
#endif
    ...
    static void menuHandler(void* ctx) {
        ...
        // Recording path
#if defined(TARGET_OS_IPHONE) && TARGET_OS_IPHONE
        // iOS: path is always <Documents>/recordings (forced in constructor).
        // The "..." file picker is unavailable (fork() is forbidden on iOS) and
        // the container UUID changes on reinstall, so we just show a static label.
        ImGui::LeftLabel("Rec Path");
        ImGui::TextDisabled("Documents/recordings");
#else
    ...
    // Per-slot power history for spur/birdie detection (DSP thread only)
    std::map<int, std::deque<float>>        slotMeanHistory;
    std::set<int>                           confirmedSpurs;
