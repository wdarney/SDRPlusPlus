...
namespace backend {
    ...
    void setFrameRateLimit(int fps) {
        int preferred = std::clamp(fps, 0, 120);
        dispatch_async(dispatch_get_main_queue(), ^{
            if (g_mtkView) g_mtkView.preferredFramesPerSecond = preferred > 0 ? preferred : 60;
        });
    }

    int getFrameRateLimit() {
        return g_mtkView ? (int)g_mtkView.preferredFramesPerSecond : 60;
    }
