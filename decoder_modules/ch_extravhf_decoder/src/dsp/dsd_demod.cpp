...
namespace dsp {
        ...
        int NewDSD::process(int count, const uint8_t* in, short* out) {
            ...
            while((count-pos) > 0) {
                int prevPos = pos;
                state prevState = curr_state;
                uint64_t prevInternalProgress = internalProgress;
                switch(curr_state) {
                ...
                if (pos == prevPos && curr_state == prevState && internalProgress == prevInternalProgress) {
                    flog::warn("DSD decoder made no progress in state {0}; dropping one dibit and resetting sync", (int)curr_state);
                    frame_status.sync = false;
                    curr_state = STATE_NO_SYNC;
                    pos++;
                }
            }
            auto writeProbeTone = [this](short* dest, int sampleCount) {
                static constexpr double PROBE_FREQ = 800.0;
                static constexpr double PROBE_SR = 8000.0;
                static constexpr double PROBE_STEP = 2.0 * M_PI * PROBE_FREQ / PROBE_SR;
                static constexpr short PROBE_AMP = 5000;
                for (int i = 0; i < sampleCount; i++) {
                    dest[i] = (short)(sin(audioProbePhase) * PROBE_AMP);
                    audioProbePhase += PROBE_STEP;
                    if (audioProbePhase >= 2.0 * M_PI) {
                        audioProbePhase -= 2.0 * M_PI;
                    }
                }
            };

            bool probeTone = audioProbeTone.load();
            if (probeTone && outcnt > 0) {
                writeProbeTone(out, outcnt);
            }
            ...
            if(remainingOut > 0 && (probeTone || !mbe_status.mbe_status_decoding)) {
                if (probeTone) {
                    writeProbeTone(&(out[outcnt]), remainingOut);
                }
                else {
