...
#include "vdl2_protocol_config.h"                 // IS_BIG_ENDIAN
#include "dumpvdl2.h"
#include "x25.h"
...
typedef struct {
    ...
    uint16_t channel; // SDR++: distinguish simultaneous virtual circuits too.
} x25_avlc_info;
...
uint32_t x25_key_hash(void const *key) {
	...
	return k->src_addr * 11 + k->dst_addr * 23 + k->channel * 31;
}

bool x25_key_compare(void const *key1, void const *key2) {
	x25_avlc_info const *k1 = key1;
	x25_avlc_info const *k2 = key2;
	return k1->src_addr == k2->src_addr && k1->dst_addr == k2->dst_addr && k1->channel == k2->channel;
}
...
static int parse_x25_callreq_sndcf(x25_pkt_t *pkt, uint8_t *buf, uint32_t len) {
	...
	if(sndcf_len < MIN_X25_SNDCF_LEN || len < sndcf_len || *buf != X25_SNDCF_VERSION) {
...
static la_proto_node *parse_x25_user_data_impl(uint8_t *buf, uint32_t len, uint32_t *msg_type,
		reasm_contexts *rtables, struct timeval rx_time, uint32_t src_addr, uint32_t dst_addr) {
	if(buf == NULL || len == 0) {
		return NULL;
	}
		...
		case X25_DATA:
			{
				...
				if(rtables->seqbased != NULL) {   // reassembly engine is enabled
					...
					x25_avlc_info avlc_info = {
                        ...
                        .channel = (hdr->chan_group << 8) | hdr->chan_num
					};
					...
					} else if(pkt->reasm_status != LA_REASM_SKIPPED) {
...
// SDR++: a malformed chain of embedded error reports must have bounded depth.
static la_proto_node *parse_x25_user_data(uint8_t *buf, uint32_t len, uint32_t *msg_type,
		reasm_contexts *rtables, struct timeval rx_time, uint32_t src_addr, uint32_t dst_addr) {
    if(rtables->parse_depth >= 16) return unknown_proto_pdu_new(buf, len);
    rtables->parse_depth++;
    la_proto_node *node = parse_x25_user_data_impl(buf, len, msg_type, rtables, rx_time, src_addr, dst_addr);
    rtables->parse_depth--;
    return node;
}
