...
static la_proto_node *parse_clnp_pdu_payload(uint8_t *buf, uint32_t len, uint32_t *msg_type,
	...
	switch(*buf) {
		...
		case SN_PROTO_IDRP:
			return unknown_proto_pdu_new(buf, len); // IDRP is outside this subset.
		case SN_PROTO_CLNP:
...
static la_proto_node *clnp_pdu_parse_impl(uint8_t *buf, uint32_t len, uint32_t *msg_type,
		reasm_contexts *rtables, struct timeval rx_time, uint32_t src_addr, uint32_t dst_addr) {
	NEW(clnp_pdu_t, pdu);
	...
	pdu->hdr = hdr;
	if(hdr->len < CLNP_MIN_LEN || hdr->len == 255) {
	...
	// Parse variable fields only within the declared header.
    if(pdu->seg_len < hdr->len || pdu->seg_len > len) goto fail;
    len = pdu->seg_len;
    ptr += sizeof(clnp_hdr_t); remaining = hdr->len - sizeof(clnp_hdr_t);
    ...
    if(options_part_len < 0) goto fail;
	...
	if(pdu->hdr->type == CLNP_NDPU_ER) {
        ...
        ptr = buf + hdr->len; remaining = len - hdr->len;
...
la_proto_node *clnp_compressed_data_pdu_parse(uint8_t *buf, uint32_t len, uint32_t *msg_type,
	...
	if(pdu->derived && rtables->offsetbased != NULL) {   // reassembly engine is enabled
		...
		if(clnp_rtable == NULL) {
			clnp_rtable = reasm_table_new(rtables->offsetbased, &proto_DEF_clnp_compressed_data_pdu,
					clnp_reasm_funcs, CLNP_REASM_TABLE_CLEANUP_INTERVAL);
		}
		struct clnp_reasm_key reasm_key = {
			.src_addr = src_addr, .dst_addr = dst_addr, .pdu_id = pdu->pdu_id
...
la_proto_node *clnp_pdu_parse(uint8_t *buf, uint32_t len, uint32_t *msg_type,
    ...
    rtables->parse_depth++;
    la_proto_node *node = clnp_pdu_parse_impl(buf, len, msg_type, rtables, rx_time, src_addr, dst_addr);
    rtables->parse_depth--;
    return node;
}
