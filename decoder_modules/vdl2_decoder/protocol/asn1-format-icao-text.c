...
// SDR++ adaptation: libacars owns temporary byte buffers; no GLib dependency.
static la_vstring *vdl2_bytes_append(la_vstring *v, uint8_t const *data, size_t len) {
    if(len > 0) la_vstring_append_buffer(v, data, len);
    return v;
}

static la_vstring *_stringify_ShortTsap(la_vstring *array, ShortTsap_t const *tsap) {
	if(tsap->aRS != NULL) {
		array = vdl2_bytes_append(array, tsap->aRS->buf, tsap->aRS->size);
	}
	array = vdl2_bytes_append(array, tsap->locSysNselTsel.buf, tsap->locSysNselTsel.size);
	return array;
}
...
static LA_ASN1_FORMATTER_FUNC(asn1_format_LongTsap_as_text) {
	...
	la_vstring *tmparray = la_vstring_new();
	tmparray = vdl2_bytes_append(tmparray, tsap->rDP.buf, tsap->rDP.size);
	tmparray = _stringify_ShortTsap(tmparray, &tsap->shortTsap);

	LA_ISPRINTF(p.vstr, p.indent, "%s: ", p.label);
	octet_string_with_ascii_format_text(p.vstr,
			&(octet_string_t){ .buf = (uint8_t *)tmparray->str, .len = tmparray->len },
			0);
	EOL(p.vstr);
	la_vstring_destroy(tmparray, true);
}
